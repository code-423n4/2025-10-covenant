...
import {StubPyth} from "./StubPyth.sol";
...
contract PythOracleHelper is AdapterHelper {
    ...
    Bounds internal DEFAULT_BOUNDS =
        Bounds({
            minBaseDecimals: 0,
            maxBaseDecimals: 18,
            minQuoteDecimals: 0,
            maxQuoteDecimals: 18,
            minInAmount: 1,
            maxInAmount: 1e18, // Much smaller max to prevent overflow
            minPrice: 1,
    ...
    function setUpState(FuzzableState memory s) internal {
        s.base = TestUtils.boundAddr(s.base);
        s.quote = TestUtils.boundAddr(s.quote);
        vm.assume(TestUtils.distinct(s.base, s.quote, PYTH));
        ...
        if (behaviors[Behavior.FeedReturnsNegativePrice]) {
            ...
            // Additional safety check to prevent extreme values that could cause overflow
            if (s.p.price <= 0) {
                s.p.price = 1;
            }
        }
        ...
        if (diff > 0) {
            return
                FixedPointMathLib.fullMulDiv(
                    s.inAmount,
                    uint256(uint64(s.p.price)) * 10 ** s.quoteDecimals,
                    10 ** (uint8(diff))
                );
        } else {
            return
                FixedPointMathLib.fullMulDiv(
                    s.inAmount,
                    uint256(uint64(s.p.price)) * 10 ** (s.quoteDecimals + uint8(-diff)),
                    1
                );
        }
    }
