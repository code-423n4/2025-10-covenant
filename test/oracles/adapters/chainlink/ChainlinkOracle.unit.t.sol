...
contract ChainlinkOracleTest is ChainlinkOracleHelper {
    ...
    function test_Quote_RevertsWhen_TooStale(FuzzableState memory s) public {
        ...
        bytes memory err = abi.encodeWithSelector(
            Errors.PriceOracle_TooStale.selector,
            s.timestamp - s.updatedAt,
            s.maxStaleness
        );
        expectRevertForAllQuotePermutations(s.inAmount, s.base, s.quote, err);
    }
    ...
    function test_PreviewGetQuote_Integrity(FuzzableState memory s) public {
        ...
        uint256 expectedOutAmount = calcOutAmount(s);
        uint256 outAmount = ChainlinkOracle(oracle).previewGetQuote(s.inAmount, s.base, s.quote);
        assertEq(outAmount, expectedOutAmount);
    }
    ...
    function test_PreviewGetQuote_Integrity_Inverse(FuzzableState memory s) public {
        ...
        uint256 expectedOutAmount = calcOutAmountInverse(s);
        uint256 outAmount = ChainlinkOracle(oracle).previewGetQuote(s.inAmount, s.quote, s.base);
        assertEq(outAmount, expectedOutAmount);
    }
    ...
    function test_PreviewGetQuotes_Integrity(FuzzableState memory s) public {
        ...
        uint256 expectedOutAmount = calcOutAmount(s);
        (uint256 bidOutAmount, uint256 askOutAmount) = ChainlinkOracle(oracle).previewGetQuotes(
            s.inAmount,
            s.base,
            s.quote
        );
    ...
    function test_UpdatePriceFeeds_RevertsWhen_ExcessValue(FuzzableState memory s) public {
        ...
        ChainlinkOracle(oracle).updatePriceFeeds{value: 1 ether}(s.base, s.quote, "");
    }

    function test_UpdatePriceFeeds_Success_WithZeroValue(FuzzableState memory s) public {
        setUpState(s);

        // Should not revert when called with zero value
        ChainlinkOracle(oracle).updatePriceFeeds(s.base, s.quote, "");
    }

    function test_GetUpdateFee_ReturnsZero(FuzzableState memory s) public {
        setUpState(s);

        uint128 fee = ChainlinkOracle(oracle).getUpdateFee(s.base, s.quote, "");
        assertEq(fee, 0);
    }
}
